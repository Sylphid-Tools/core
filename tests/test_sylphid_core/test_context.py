...
def patched_constants(monkeypatch):
    monkeypatch.setattr(
        context.constants, "CTX_ENV_KEY_ORDER", ["prj", "seq", "sht", "tsk"]
    )
    monkeypatch.setattr(
        context.constants, "CTX_DICT_KEY_ORDER", ["prj", "seq", "sht", "tsk"]
    )
...
class TestValidateContext:
    @pytest.mark.parametrize(
        ...
        [
            ...
            {
                "seq": "seq"
            },  # the first field, in this case prj, must always be populated
            {
                "prj": "prj",
                "seq": "seq",
                "tsk": "tsk",
            },  # tsk is dependant on sht which is unpopulated
            {
                "prj": "prj",
                "seq": "seq",
                "sht": None,
                "tsk": "tsk",
            },  # tsk is dependant on sht which is None
            {
                "prj": "prj",
                "seq": "seq",
                "sht": "",
                "tsk": "tsk",
            },  # tsk is dependant on sht which is empty
        ],
    )
...
class TestFromEnvironment:
    def test_extraction(self, monkeypatch, patched_constants):
        ...
        assert ctx == {"prj": "a", "seq": "b", "sht": "c", "tsk": "d"}
